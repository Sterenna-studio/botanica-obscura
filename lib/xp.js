...
export const LEVEL_TABLE = [
  ...
  { level: 11, xpRequired: 8000,  reward: { coins: 600,  label: '+600🪙 +1 slot 🪨', potSlots: 1 } },
  { level: 12, xpRequired: 10500, reward: { coins: 700,  label: '+700🪙',            potSlots: 0 } },
  { level: 13, xpRequired: 13500, reward: { coins: 900,  label: '+900🪙',            potSlots: 0 } },
  { level: 14, xpRequired: 17000, reward: { coins: 1200, label: '+1200🪙 +1 slot 🪨', potSlots: 1 } },
  { level: 15, xpRequired: 21000, reward: { coins: 2000, label: '+2000🪙 🏆',         potSlots: 0 } },
];
...
/**
 * Octroie des récompenses au joueur : XP (avec montée de niveau et bonus de palier)
 * et pièces directes (quêtes, bonus quotidien…). Sauve en local + sync cloud.
 *
 * @param {string} userId
 * @param {object} currentData      { xp, coins, level, pot_slots }
 * @param {object} rewards          { xp?:number, coins?:number }
 * @returns {{ newXp, newLevel, newCoins, newPotSlots, leveledUp, reward }}
 */
export async function grantPlayerRewards(userId, currentData, { xp = 0, coins = 0 } = {}) {
  const prevTotal = currentData.xp    ?? 0;
  const prevCoins = currentData.coins ?? 0;
  const prevLevel = currentData.level ?? 1;

  const newTotal = prevTotal + Math.max(0, xp);
  ...
  const newCoins    = prevCoins + bonusCoins + Math.max(0, coins);
...
export async function addXpToPlayer(userId, xpGained, currentData) {
  return grantPlayerRewards(userId, currentData, { xp: xpGained });
}
