...
/**
 * Calcule les poids de qualité (mêmes décalages que rollQualityTier) puis
 * renvoie la probabilité (0..1) de chaque tier — utilisé par les panneaux d'info.
 * @param {Object} gardenBonuses
 * @returns {Array<{tier, probability}>} aligné sur QUALITY_TIERS
 */
export function qualityOdds(gardenBonuses = {}) {
  const {
    waterBonus  = 0,
    lightBonus  = 0,
    thermoBonus = 0,
  ...
  return QUALITY_TIERS.map((tier, i) => ({ tier, probability: clamped[i] / total }));
}
